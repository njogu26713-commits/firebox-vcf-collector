  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <h3 className="text-lg font-bold text-white mb-6">Contacts Collected per Campaign</h3>
                ...
                <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 20 }}>
                  ...
                  <Tooltip 
                    ...
                    content={({ active, payload }) => {
                      if (active && payload && payload.length) {
                        return (
                          <div className="bg-[#111] border border-border p-3 rounded-lg shadow-xl">
                            <p className="text-white font-medium mb-1">{payload[0].payload.fullTitle}</p>
                  ...
                  <Bar dataKey="collected" radius={[4, 4, 0, 0]}>
                    {chartData.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.percent >= 100 ? '#22c55e' : '#FF6A00'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
        ...
        <motion.div 
          ...
          className="bg-card border border-border rounded-[20px] p-6 h-[400px] flex flex-col"
        >
          <h3 className="text-lg font-bold text-white mb-6">Completion Progress (%)</h3>
          <div className="flex-1 min-h-0 w-full">
            {isLoading ? (
              <Skeleton className="w-full h-full bg-secondary/50 rounded-lg" />
            ) : chartData.length === 0 ? (
      ...
      >
        <div className="p-6 border-b border-border">
          <h3 className="text-lg font-bold text-white">Campaign Breakdown</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            <thead className="text-xs text-muted-foreground bg-[#0B0B0B]/50 uppercase">
              <tr>
              ...
              {isLoading ? (
                ...
                analytics?.map((c) => (
                  <tr key={c.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right font-bold text-white">{c.contactsCollected.toLocaleString()}</td>
